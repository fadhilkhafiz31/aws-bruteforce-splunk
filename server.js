...
// Stable event IDs — SIEM/SOC analysts pivot on these
const EVENT_ID_MAP = {
  LOGIN_SUCCESS:         'AUTH-200',
  LOGIN_FAILED:          'AUTH-401',
  LOGIN_ERROR:           'AUTH-500',
  REGISTER_SUCCESS:      'AUTH-201',
  REGISTER_FAILED:       'AUTH-409',
  LOGOUT:                'AUTH-LOGOUT',
  ADMIN_ACCESS:          'ADMIN-200',
  UNAUTHORIZED_ACCESS:   'ADMIN-403',
  TOKEN_REPLAY_DETECTED: 'THREAT-REPLAY',
  TRANSACTIONS_ACCESSED: 'TXN-200',
  OAUTH_TOKEN_SUCCESS:   'OAUTH-200',
  OAUTH_TOKEN_FAILED:    'OAUTH-401'
};
...
const SEVERITY_MAP = {
  ...
  // anything not listed defaults to 'low'
};
...
function securityLog(req, event) {
  const enriched = {
    timestamp: new Date().toISOString().slice(0, 23).replace('T', ' '),
    event_id:  EVENT_ID_MAP[event.event_type] || 'UNKNOWN',
    severity:  SEVERITY_MAP[event.event_type] || 'low',
  ...
  // 2. Write to MySQL — extra fields land in metadata column
  db.logSecurityEvent(enriched);
}
